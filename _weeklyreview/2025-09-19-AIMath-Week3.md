...
{[K=1, K=3, K=5일 때의 K-NN 결정 경계 변화를 보여주는 이미지]}
...
{[손실 함수의 표면에서 경사 하강법으로 최저점을 찾아가는 과정을 시각화한 이미지]}
...
{[입력층, 은닉층, 출력층으로 구성된 MLP 구조 다이어그램 이미지]}
...
{[Computational Graph를 이용한 역전파 과정 설명 이미지]}
...
{[다양한 학습률 스케줄링 방법에 따른 학습률 변화 그래프 이미지]}
...
{[이미지 데이터 증강 기법(회전, 자르기, 뒤집기 등) 예시 이미지]}
...
{[LSTM 셀의 내부 구조(Forget, Input, Output 게이트 포함) 이미지]}
...
{[어텐션 메커니즘이 번역 작업에서 입력 단어와 출력 단어의 관계를 정렬하는 것을 보여주는 히트맵 이미지]}

### 5.2. 트랜스포머 아키텍처

트랜스포머는 크게 인코더와 디코더, 두 부분으로 구성되며, 각각 여러 개의 동일한 블록을 쌓은 구조를 가진다.

{[트랜스포머의 전체 아키텍처 다이어그램(인코더-디코더, 멀티 헤드 어텐션, 위치 인코딩 포함)]}
