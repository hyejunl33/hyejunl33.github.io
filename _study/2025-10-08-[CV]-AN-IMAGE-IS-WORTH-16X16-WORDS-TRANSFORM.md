...
일반적으로 ViT는 대규모 데이터셋에서 사전 훈련된 후, 더 작은 Downstream task에 Fine-tuning된다. 이때 사전 훈련에 사용된 예측 헤드를 제거하고, 목표 클래스 수(K)에 맞는 새로운, 0으로 초기화된 $D×K$ *FeedForward layer*를 부착한다.
